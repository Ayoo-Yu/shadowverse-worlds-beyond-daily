...
3. 使用官方 X Recent Search 执行日文、英文查询，并把 `config/sources.json` 中未过期的固定与动态账号合并为精确 `from:` 查询。只有翻页到末尾且响应没有未解决错误时才记为 `verified`；达到页数上限仍有 `next_token` 必须记为 `partial`。公开索引只能发现候选，不能替代这三项官方覆盖。
...
- `verified`：精确账号/查询成功读取，观察窗口、作者或 UID 可以复核；X查询还必须完整翻页且无未解决响应错误。
...
发布脚本默认先运行 B站逐 UID 采集，再运行三项官方 X 采集，最后执行同一质量校验。`X_BEARER_TOKEN` 必须来自有余额的 X Developer App；脚本可读取当前进程或 Windows 用户级环境变量。只有离线重放一份已经采集完毕的快照时才使用 `-SkipBilibiliCollect -SkipXCollect`。缺失任何固定检查项、翻页未完成、账号 UID 不一致、覆盖数字与逐项审计不一致，都会阻止推荐或中止构建。
